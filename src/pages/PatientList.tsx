...
interface PatientListProps {
  /** Passes the whole selected patient, not just its id — this list already
      holds the full PatientListItem in memory (usePatients()), so the chart
      page (PatientChart.tsx's new Frame 2) can show it immediately with no
      extra fetch. `label` stays a separate argument (not recomputed from
      `patient` by every caller) since App.tsx's header also needs it. */
        ...
        {showNewForm && (
          <NewPatientForm
            onCreated={(patient) => {
              ...
              onSelectPatient(patient, patientLabel(patient));
            }}
            ...
            {filtered.map((p) => {
              ...
              return (
                <li key={p.patientId}>
                  <button
                    type="button"
                    onClick={() => onSelectPatient(p, patientLabel(p))}
